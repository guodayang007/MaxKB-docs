...
!!! Abstract ""
    **版本号说明：** 像其它优秀开源项目一样，GTAI 将保持每月发布一个新的功能版本，功能版本中如遇较为紧急或严重的 Bug，将及时推出 Bug 修复的小版本。           
    **GTAI 版本号命名规则为：** v 大版本.功能版本. Bug 修复版本，示例如下：
...
!!! Abstract "问题修复 :palm_tree:"
    ...
    - 工具库：修复当工具参数是非字符类型并且值为空时，运行工具报错的问题；
    - 工具库：修复先禁用再启用工具后，工具的部分启用参数会被变更“X” 的问题；
...
!!! Abstract "功能优化 :sunflower:"
    - 应用：文件上传支持上传其他自定义的文件类型，该类型文件需要自行写工具解析。
    - 问答页面：支持拖拽文件到问题输入框。
    - 问答页面：支持复制粘贴文件和内容到问题输入框。

!!! Abstract "问题修复 :palm_tree:"
    - 应用：修复通过 openai 接口进行对话时，携带 asker 参数不生效的问题。
    - 应用：修复工作流应用中，部分情况下添加应用节点会显示不可用的问题。
...
!!! Abstract "功能优化 :sunflower:"
    - 应用： 在工作流的判断器节点，支持通过拖拽方式调整 IF、ELSE IF 分支的顺序。
    - 应用：在工作流的判断器节点，判断条件的值支持设置为真和不为真。
    - 应用：在工作流的基础信息节点，优化接口参数的默认值较长时的显示样式问题。
    - 登录认证（X-Pack）：优化企业微信免密登录时优先获取用户的企业邮箱。

!!! Abstract "问题修复 :palm_tree:"
    - 安全：解决工具库模块中反弹 Shell 的漏洞（CVE-2025-3238）。
...
!!! Abstract "新增功能 :star2:"
    - 应用：新增 MCP 调用节点，该节点具备充当远程工具库服务的能力，用户可以从 MCP 服务所提供的工具集中选择工具进行调用。
    - 应用：AI 对话节点新增工具调用功能，用户在工具设置中开启并完成 MCP 工具的配置后，大语言模型将根据用户提出的问题，自主调用最为合适的MCP工具来处理任务。
    - 应用：嵌入第三方功能支持移动端模式。
    - 应用（X-Pack）：在显示设置中，支持设置是否显示用户提问头像以及 AI 回复头像。
    - 工具库：新增内置工具库，包括 Google 搜索、Bocha 搜索（博查搜索）、LangSearch、MySQL 查询以及 PostgreSQL 查询。
    - 工具库：工具新增启动参数配置。
    - 工具库：支持设置工具的 Logo。
...
!!! Abstract "问题修复 :palm_tree:"
    ...
    - 应用：修复工作流中的提示词窗口放大后编辑内容，按ESC键关闭窗口后不保存提示词的问题。
    ...
    - 工具库：修复工具返回值为0时，调试时的输出结果显示错误的问题。
...
!!! Abstract "功能优化 :sunflower:"
    - 应用：优化工作流应用中当节点数过多时，打开工作流编排页面较慢的问题。
    - 应用：优化发送给AI模型的历史聊天记录中过滤掉表单数据，避免影响AI回答。
    - 应用：优化输出思考设置的描述。
    - 应用：优化工作流应用中，调试预览的对话窗口只有点击“关闭”按钮时才会被关闭。
...
!!! Abstract "新增功能 :star2:"
    - 应用：针对DeepSeek R1深度思考功能的发布，GTAI对应新增“输出思考”配置功能。
    - 应用：鉴于不同供应商所提供的思考过程标识存在差异，GTAI支持用户依据所选用的具体模型，自定义思考过程的标签。
    - 应用：支持语音输入自动发送。
    - 应用：支持语音自动播放。
    - 应用：显示设置中新增问答页面的语言设置。
...
!!! Abstract "问题修复 :palm_tree:"
    - 安全：修复工具库模块中远程命令执行的安全漏洞（CVE-2024-56137）。
...
!!! Abstract "功能优化 :sunflower:"
    - 应用：工作流应用中保存节点的收起/展开状态。

!!! Abstract "问题修复 :palm_tree:"
    - 应用：修复应用接入认证过的公众号，提问时不回复的问题。
    - 应用：修复应用接入公众号配置页面的样式问题。
    - 应用：工具中设置了非必填参数，在应用中引用该工具时，若未传值执行会报错的问题。
...
!!! Abstract "新增功能 :star2:"
    ...
    - 【应用】支持查看和恢复历史版本。         
    - 【应用】支持设置对话日志的清除策略。         
    - 【应用】工作流应用支持自动保存设置，默认不开启，开启后默认5分钟保存一次。         
...
!!! Abstract "功能优化 :sunflower:"
    ...
    - 【应用】导出工作流应用时包含知识库检索节点的引用分段数据。         
    - 【应用】在语音输入时，控制台不再输出调试信息。         
    - 【工具库】优化工具中 Python 编辑器组件，使得编写工具体验更友好。         
    - 【团队管理】设置资源权限时增加全选按钮。         
...
!!! Abstract "问题修复 :palm_tree:"
    ...
    - 【应用】修复升级后第一次进入工作流应用编辑页面，显示空白的问题。         
    - 【应用】修复工作流应用设置全局变量后，打开调试对话框，选择变量后调试对话框会被关闭的问题。          
...
!!! Abstract "功能优化 :sunflower:"
    ...
    - 【应用】工作流应用的节点支持展开和收起，节省画布空间。         
    - 【应用】工作流应用点击节点右侧“+ ”号可直接添加节点。         
...
!!! Abstract "问题修复 :palm_tree:"
    - 【知识库】修复向GTAI上传语雀导出的Word文档时，导入内容是空白的问题。         
...
!!! Abstract "问题修复 :palm_tree:"
    ...
    - 【应用】修复当应用只有使用权限，在工作流页面一直提示没有权限的问题。             
    - 【知识库】修复修改关联问题报错的问题。                
...
!!! Abstract "新增功能 :star2:"
    ...
    - 【应用】工作流的指定回复节点支持快捷问题输出：`<quick_question>快捷问题</quick_question>`。             
...
!!! Abstract "新增功能 :star2:"
    - 【应用】支持创建简易配置和工作流两种类型的应用，工作流应用支持自定义工作流。                
    - 【应用】工作流的应用AI对话节点支持角色设定。             
    - 【应用】工作流的应用AI对话节点支持自定义设置携带的历史聊天记录数。             
    - 【应用】工作流的应用支持获取当前时间的系统变量。             
    - 【对话日志】工作流应用支持查看每个节点的执行详情。 
...
!!! Abstract "问题修复 :palm_tree:"
    ...
    - 【模型管理】Ollama 模型中没有已下载的模型时，在 GTAI 添加 Ollama 模型报错。            
    - 【模型管理】添加模型时，上次的表单未清空。
...
!!! Abstract "问题修复 :palm_tree:"
    ...
    - 【用户管理】修复创建用户时登录密码使用默认密码GTAI@123.. 提示规则不对。         
